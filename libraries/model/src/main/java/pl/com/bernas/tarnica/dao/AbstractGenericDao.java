...
public abstract class AbstractGenericDao<T extends TarnicaEntity, PK extends Serializable> implements GenericDao<T, Long> {
	...
	protected T executeQueryWithUniqueResult(String query, QueryParam... params) {
		return (T) new QueryExecutor(primaryEm, query, Arrays.asList(params), true).execute();
	}
	...
	private class QueryExecutor {
		...
		QueryExecutor(EntityManager em, String queryString, List<QueryParam<? extends Object>> params, boolean uniqueResult) {
			this.em = em;
		...
		public Object execute() {
			...
			if (params != null && params.size() > 0) {
				params.forEach(param -> addParameterToQuery(query, param));
			}
		...
		public void addParameterToQuery(Query query, QueryParam<? extends Object> param) {
